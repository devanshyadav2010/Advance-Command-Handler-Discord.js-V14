...
module.exports = {
  async execute(client) {
    let djs = `Welcome to ${'Server Handler'.bold.blue} by ${'ALL IN ONE | Development'.red}`;
    let djs_support = `Support:- ${`https://discord.gg/pXRT2FusPb`.brightGreen}`
    let djs_devs = `Coded By ${`Uo#1428`.brightCyan.bold}`
      let djs_devs2 = `Modified By ${`DEVANSH#4075`.brightCyan.bold}`;
